...
const resolveType = (
  ...
  type.has("type")
    ...
    ? schema
        ...
        .find(
          (def: any) =>
            def.getIn(["name", "value"]) === type.getIn(["name", "value"])
        )
...
const validators = immutable.fromJS({
  ...
  UnionTypeDefinition: (value: any, ctx: immutable.Map<any, any>) => {
    ...
    return List(
      errors.minBy((variantErrors: any) => variantErrors.count()) as any
    );
  },
  ...
  Boolean: (value: any, ctx: immutable.Map<any, any>) =>
    typeof value === "boolean"
      ...
      : immutable.fromJS([
          {
            path: ctx.get("path"),
            message: `Expected true or false, found ${typeName(
              value
            )}:\n${toYAML(value)}`,
          },
        ]),
...
const dataSourceListToMap = (dataSources: any[]) =>
  dataSources.reduce(
    (protocolKinds, dataSource) =>
      protocolKinds.update(
        Protocol.normalizeName(dataSource.kind),
        (networks: any) =>
  ...
  const errors =
    value !== null && value !== undefined
      ...
      : immutable.fromJS([
          {
            path: [],
            message: `Expected non-empty value, found ${typeName(
              value
            )}:\n  ${value}`,
          },
        ]);
