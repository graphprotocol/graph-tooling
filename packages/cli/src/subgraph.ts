...
const throwCombinedError = (
  filename: string,
  errors: Array<{ path: any[]; message: string }>
) => {
  throw new Error(
    errors.reduce(
      (msg, e) =>
...
export default class Subgraph {
  ...
  static validateSchema(
    ...
    if (validationErrors.size > 0) {
      ...
      const msg = errors.reduce((msg, errors, entity) => {
        ...
        const inner_msgs = errors.reduce(
  ...
  static validateRepository(manifest: ManifestZodSchema) {
    ...
    return /^https:\/\/github\.com\/graphprotocol\/graph-tooling?$/.test(
      repository
    ) ||
      // For legacy reasons, we should error on example subgraphs
      /^https:\/\/github\.com\/graphprotocol\/example-subgraphs?$/.test(
        repository
      )
      ? [
          {
...
At least one such handler must be defined.`,
  ...
  static validateUniqueDataSourceNames(manifest: ManifestZodSchema) {
    ...
    return manifest.dataSources.reduce(
      (errors: any, dataSource, dataSourceIndex) => {
...
More than one data source named '${name}', data source names must be unique.`,
  ...
  static validateUniqueTemplateNames(manifest: ManifestZodSchema) {
    const names: string[] = [];
    return (manifest?.templates || []).reduce(
      (errors: any, template, templateIndex) => {
...
More than one template named '${name}', template names must be unique.`,
  ...
  static async load(
    filename: string,
    {
      protocol,
      skipValidation,
    }: { protocol?: any; skipValidation?: boolean } = {
    ...
    return {
      result: manifest,
      warning:
        warnings.length > 0 ? buildCombinedWarning(filename, warnings) : null,
    };
  }
