...
import withSpinner, { step } from "./spinner";
...
export const updateSubgraphNetwork = async (
  ...
  await withSpinner(
    ...
    async (spinner) => {
      ...
      if (!networkConfig) {
        throw new Error(
          `Network '${network}' was not found in '${networksFile}'`
        );
      }
      ...
      await patching.update(manifest, (content) => {
        ...
        const subgraphSources = subgraph.dataSources.map(
        ...
        subgraph.dataSources = subgraph.dataSources.map((source: any) => {
          ...
          if (
            ...
            step(
              spinner,
              `Skip '${source.name}': No changes to network configuration`
            );
          }
        ...
        const unusedSources = networkSources.filter(
          (x) => !subgraphSources.includes(x)
        );
...
export const initNetworksConfig = async (
  ...
  await withSpinner(
    ...
    async () => {
      const subgraphStr = filesystem.read(
...
function hasChanges(
  ...
  const addressChanged =
    networkConfig[identifierName] !== dataSource.source[identifierName];

  const startBlockChanged =
    networkConfig.startBlock !== dataSource.source.startBlock;
