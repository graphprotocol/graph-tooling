...
export default class ABI {
  ...
  callFunctions() {
    ...
    const functionTypes = immutable.Set([
      "constructor",
      "function",
      "fallback",
    ]);
    const functions = this.data.filter(
      (entry: any) =>
        !entry.has("type") || functionTypes.includes(entry.get("type"))
    );
  ...
  static load(name: string, file: string) {
    ...
    if (abi === null || abi === undefined) {
      throw Error(
        `No valid ABI in file: ${path.relative(process.cwd(), file)}`
      );
    }
