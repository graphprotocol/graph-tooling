...
use crate::logging;
use crate::parser;
use crate::{LIBS_LOCATION, MANIFEST_LOCATION, TESTS_LOCATION};
...
fn collect_wasm_files(path: &Path) -> Vec<PathBuf> {
  ...
  let entries = path
    .read_dir()
    .unwrap_or_else(|err| logging::critical!("Could not get wasm files from {:?}: {}", path, err));
