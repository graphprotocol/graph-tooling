...
mod tests {
  ...
  fn prepare_entity_store_pointers(
    ...
    let map_entry_pointer = AscPtr::alloc_obj(map_entry, &mut context.wasm_ctx, &GasCounter::new())
      .expect("Couldn't create pointer.");
  ...
  fn mock_store_load_related_basic_test() {
    ...
    let related_entities: Vec<HashMap<String, Value>> = asc_get::<_, Array<AscPtr<AscEntity>>, _>(
      &context.wasm_ctx,
  ...
  fn ethereum_call_basic_test() {
    ...
    let address_pointer = AscPtr::alloc_obj(address, &mut context.wasm_ctx, &GasCounter::new())
      .expect("Couldn't create pointer.");
    let func_name_pointer = AscPtr::alloc_obj(func_name, &mut context.wasm_ctx, &GasCounter::new())
      .expect("Couldn't create pointer.");
    ...
    let func_args_pointer = AscPtr::alloc_obj(asc_enum, &mut context.wasm_ctx, &GasCounter::new())
      .expect("Couldn't create pointer.");
  ...
  fn mock_ipfs_map() {
    ...
    let user_data_ptr = asc_new(&mut context.wasm_ctx, &user_data, &GasCounter::new())
      .expect("Couldn't create pointer.");
    let flags_ptr =
      asc_new(&mut context.wasm_ctx, &flags, &GasCounter::new()).expect("Couldn't create pointer.");
