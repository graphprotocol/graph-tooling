...
impl Test {
  ...
  pub fn call_hooks(hooks: &[Func]) {
    hooks.iter().for_each(|h| {
      h.call(&[])
        .unwrap_or_else(|err| logging::critical!("Unexpected error upon calling hook: {}", err));
    });
  }
  ...
  for (t_name, should_fail, t_idx, role) in functions {
    ...
    match role.as_str() {
      ...
      "describe" => {
        ...
        let nested_test_group = build_test_group(matchstick, &t_name, nested_functions, table);
        test_group
          .testables
