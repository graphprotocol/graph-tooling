...
use graph::{
  ...
  runtime::{
    asc_get, asc_new, gas::GasCounter, AscIndexId, AscPtr, AscType, FromAscObj, HostExportError,
    ToAscObj,
  },
  semver::Version,
};
...
impl<C: Blockchain> MatchstickInstanceContext<C> {
  ...
  fn fn_id(contract_address: &str, fn_name: &str, fn_signature: &str, fn_args: &[Token]) -> String {
...
impl<C: Blockchain> MatchstickInstanceContext<C> {
  ...
  pub fn log_store(&mut self, _gas: &GasCounter) -> Result<(), HostExportError> {
    let string_pretty = to_string_pretty(&self.store).unwrap_or_else(|err| logging::critical!(err));
      ...
      for (derived_from_entity_type, derived_from_entity_field) in entities.iter() {
        ...
        if let Some(derived_entities) = derived_entities {
          ...
          for (derived_entity_id, derived_entity) in derived_entities.iter() {
            ...
            if relation_found {
              related_entities.push(derived_entities.get(derived_entity_id).unwrap().clone());
            }
          }
        }
      }
  ...
  pub fn ethereum_call(
    ...
    let call: UnresolvedContractCall =
      self.resolve_asc_pointer::<_, AscUnresolvedContractCall_0_0_4>(contract_call_ptr.into());
    ...
    let fn_id =
      MatchstickInstanceContext::<C>::fn_id(&contract_address, &fn_name, &fn_signature, &fn_args);
  ...
  pub fn mock_ipfs_map(
    ...
    for value in json_values {
      ...
      instance.instance_ctx_mut().data_source_return_value = self.data_source_return_value.clone();
      ...
      self.data_source_return_value = instance.instance_ctx().data_source_return_value.clone();
    }
