...
impl<C: Blockchain> MatchstickInstance<C> {
  ...
  pub fn from_valid_module_with_ctx(
    ...
    let shared_ctx: Rc<RefCell<Option<MatchstickInstanceContext<C>>>> = Rc::new(RefCell::new(None));
    ...
    if let Some(timeout) = timeout {
      ...
      graph::spawn_allow_panic(async move {
        ...
        loop {
          let time_left = timeout.checked_sub(timeout_stopwatch.lock().unwrap().elapsed());
          match time_left {
      ...
      for module in modules {
        ...
        linker.func(module, host_fn.name, move |call_ptr: u32| {
          ...
          let instance = match &mut *instance {
            ...
            None => {
              return Err(anyhow!("{} is not allowed in global variables", host_fn.name).into())
            }
          };

          let name_for_metrics = host_fn.name.replace('.', "_");
          let stopwatch = &instance.wasm_ctx.host_metrics.stopwatch;
          let _section = stopwatch.start_section(&format!("host_export_{name_for_metrics}"));
          ...
          instance
            .wasm_ctx
            .host_metrics
            .observe_host_fn_execution_time(start.elapsed().as_secs_f64(), &name_for_metrics);
          Ok(ret)
        })?;
      }
    }
