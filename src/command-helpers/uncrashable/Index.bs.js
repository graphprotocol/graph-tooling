...
var UncrashableFileNotFound = /* @__PURE__ */ Caml_exceptions.create(
  'Index.UncrashableFileNotFound',
)
...
function getDefaultValueForType(
  ...
  return Belt_Option.mapWithDefault(
    Js_dict.get(entitiesMap, typeName),
    Belt_Option.mapWithDefault(
      Js_dict.get(enumsMap, typeName),
      GraphEntityGenTemplates.getDefaultValues(typeName),
      function($$enum) {
...
function getFieldDefaultTypeNonNull(
  strictMode,
  recersivelyCreateUncreatedEntities,
  _field,
) {
...
function getFieldDefaultTypeWithNull(
  strictModeOpt,
  recersivelyCreateUncreatedEntitiesOpt,
  field,
) {
  var strictMode = strictModeOpt !== undefined ? strictModeOpt : true
  var recersivelyCreateUncreatedEntities =
    recersivelyCreateUncreatedEntitiesOpt !== undefined
      ? recersivelyCreateUncreatedEntitiesOpt
      : false
  var uncaught = field.kind
  if (uncaught === 'NonNullType') {
    return getFieldDefaultTypeNonNull(
      strictMode,
      recersivelyCreateUncreatedEntities,
      field.type,
    )
...
function generateUncrashableEntities(entityDefinitions, uncrashableConfig, safeMode) {
  ...
  var entityPrefixConfig = Belt_Option.getWithDefault(
    uncrashableConfig.networkConfig.entityIdPrefixes,
    [],
  )
  var entityPrefixDefinition =
    entityPrefixConfig.length > 1
      ? '  if ' +
        Belt_Array.joinWith(
          Belt_Array.map(entityPrefixConfig, function(param) {
            return (
              '(' +
              Belt_Array.joinWith(
                Belt_Array.map(param.networks, function(network) {
        ...
        ' else {\n    return "";\n  }'
      : '  return "";'
  var functions = Belt_Array.joinWith(
    Belt_Array.map(Object.keys(entitiesMap), function(entityName) {
      ...
      var entityConfig = Belt_Option.getWithDefault(
        Js_dict.get(uncrashableConfig.entitySettings, name),
        {
          useDefault: {},
          entityId: undefined,
          setters: undefined,
      ...
      var fieldDefaultSettersStrict = Belt_Array.joinWith(
        Belt_Array.map(fields, function(field) {
          var fieldName = field.name.value
          if (fieldName === 'id') {
            return ''
          } else {
            return GraphEntityGenTemplates.setInitializeFieldValue(
              name,
              fieldName,
              getFieldDefaultTypeWithNull(undefined, true, field.type),
            )
          }
        }),
        '\n  ',
      ...
      var fieldInitialValueSettersStrict = Belt_Array.joinWith(
        Belt_Array.map(fields, function(field) {
          var fieldName = field.name.value
          if (fieldName === 'id') {
          ...
          return Belt_Option.mapWithDefault(
            Js_dict.get(fieldsWithDefaultValueLookup, fieldName),
            GraphEntityGenTemplates.setInitializeFieldValue(
              name,
              fieldName,
              fieldNameOrEntityIds,
            ),
            function(_fieldDefaultConfig) {
              return GraphEntityGenTemplates.setInitializeFieldValue(
                name,
                fieldName,
                getFieldDefaultTypeWithNull(false, undefined, field.type),
              )
            },
          )
        }),
      ...
      var fieldToFieldTyping = function(field) {
        ...
        if (fieldName === 'id') {
          ...
          return Belt_Option.mapWithDefault(
            Js_dict.get(fieldsWithDefaultValueLookup, fieldName),
            GraphEntityGenTemplates.setFieldNameToFieldType(
              fieldName,
              getFieldType(true, field.type),
            ),
            function(param) {
              return ''
            },
      ...
      var fieldInitialValues = Belt_Array.joinWith(
        Belt_Array.map(fields, fieldToFieldTyping),
        '',
        function(a) {
          return a
        },
      )
      var idGeneratorFunction = Belt_Option.getWithDefault(
        Belt_Option.map(entityConfig.entityId, function(idArgs) {
          var argsDefinition = Belt_Array.joinWith(
            Belt_Array.keep(idArgs, function(arg) {
      ...
      var fieldSetterFunctions = Belt_Option.getWithDefault(
        Belt_Option.map(entityConfig.setters, function(setterFunctions) {
          return Belt_Array.joinWith(
            Belt_Array.map(setterFunctions, function(setter) {
              ...
              var fieldTypeDef = Belt_Array.joinWith(
                Belt_Array.map(functionSetterFields, function(field) {
                  return Belt_Option.mapWithDefault(
                    Js_dict.get(fieldsMap, field),
                    GraphEntityGenTemplates.fieldNotFoundForEntity(
                      field,
                      functionName,
                      name,
                    ),
                    function(fieldDefinition) {
                      return GraphEntityGenTemplates.setField(
                        field,
                        getFieldType(true, fieldDefinition.type),
                      )
                    },
                  )
                }),
              ...
              var fieldTypeSetters = Belt_Array.joinWith(
                Belt_Array.map(functionSetterFields, function(field) {
                  return Belt_Option.mapWithDefault(
                    Js_dict.get(fieldsMap, field),
                    GraphEntityGenTemplates.fieldNotFoundForEntity(
              ...
              return GraphEntityGenTemplates.createSetterFunction(
                functionName,
                fieldTypeDef,
                name,
                fieldTypeSetters,
      ...
      return GraphEntityGenTemplates.entityGeneratedCode(
        idGeneratorFunction,
        name,
  ...
  var entityImports = Belt_Array.joinWith(
    Belt_Array.map(
      Belt_Array.keep(entityDefinitions, function(entity) {
        if (
  ...
  return GraphEntityGenTemplates.outputCode(
    entityImports,
    entityPrefixDefinition,
    functions,
  )
}
